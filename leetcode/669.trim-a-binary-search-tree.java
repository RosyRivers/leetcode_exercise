...
class Solution {
    ...
    public TreeNode travel(TreeNode node, int low, int high) {
        if (node == null) return null;
        // 后序
        if (node.val >= low && node.val <= high) {
            node.left = travel(node.left, low, high);
            node.right = travel(node.right, low, high);
            return node;
        }
        else if (node.val < low) {
            return travel(node.right, low, high);
        }
        else {
            return travel(node.left, low, high);
        }
